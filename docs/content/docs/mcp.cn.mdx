...
### 桌面端 HTTP MCP（Windows/WSL 推荐）

DBX 桌面端启动后会启动 Streamable HTTP MCP 服务。服务只在 DBX 运行期间可用，默认地址：
...
WSL 中访问 Windows DBX 时，先在 Windows 启动 DBX 前设置：

```powershell
$env:DBX_MCP_HOST = "0.0.0.0"
```

然后在 WSL 的 MCP 客户端里使用：

```text
http://<windows-host-ip>:7424/mcp
```

不要把未授权的 MCP 能力暴露到局域网。DBX 的 MCP 工具可以读取连接信息并执行 SQL 查询。
...
桌面端 HTTP MCP 直接复用 DBX 当前的 SQLite 存储和运行时连接状态。npm stdio MCP 也会从 DBX 的 SQLite 数据库读取连接信息：
...
桌面端 HTTP MCP 会直接通过 Tauri 事件通知前端打开 tab。npm stdio MCP 的 `dbx_open_table` 和 `dbx_execute_and_show` 通过本地 HTTP 桥接接口与运行中的 DBX 应用通信：
...
- 桌面端 HTTP MCP 需要 DBX 正在运行
- npm stdio MCP 需要 Node.js 18+
- npm stdio MCP 的 UI 联动功能需要 DBX v0.3.9+
