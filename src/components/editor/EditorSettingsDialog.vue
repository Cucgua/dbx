...
import { Copy, FolderOpen, Loader2, RefreshCw, Settings, Trash2 } from "lucide-vue-next";
...
function hasAppearanceChanges(): boolean {
  return editAppLayout.value !== settingsStore.editorSettings.appLayout;
}
    ...
    <DialogContent class="sm:max-w-[720px] max-h-[calc(100vh-80px)] overflow-y-auto overflow-x-hidden">
      ...
      <Tabs v-model="activeSettingsTab">
        ...
        <TabsContent value="appearance" class="space-y-5 py-2">
          ...
          <DialogFooter class="border-t-0 bg-transparent gap-3 sm:gap-3">
            <Button variant="outline" @click="resetAppearanceDefaults">
              {{ t("settings.resetDefaults") }}
            </Button>
            <div class="flex-1" />
            <Button variant="outline" @click="emit('update:open', false)">
            ...
            <Button :disabled="!hasAppearanceChanges()" @click="applyAppearanceSettings">
              {{ t("settings.apply") }}
            </Button>
          </DialogFooter>
        </TabsContent>
