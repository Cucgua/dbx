...
const commandName = (name) => (isWindows ? `node_modules\\.bin\\${name}.cmd` : name);
...
function spawnArgs(task) {
  if (!isWindows) return { command: task.command, args: task.args };
  return {
    command: "cmd.exe",
    args: ["/d", "/s", "/c", [task.command, ...task.args].map(quoteWindowsArg).join(" ")],
  };
}
...
const tasks = [
  {
    name: "format",
    command: commandName("oxfmt"),
    args: ["--check", "apps/desktop/src/**/*.{ts,vue}"],
  },
  {
    name: "lint",
    command: commandName("oxlint"),
    args: ["--vue-plugin", "apps/desktop/src"],
  },
  {
    name: "typecheck",
    command: commandName("vue-tsc"),
...
function runTask(task) {
  ...
  const spawnConfig = spawnArgs(task);
