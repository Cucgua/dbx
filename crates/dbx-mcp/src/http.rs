...
use crate::config::{clear_status, write_status, McpHttpConfig};
...
pub async fn start_http_server(options: McpRuntimeOptions) -> Result<(), String> {
    let config = McpHttpConfig::load(&options.app_data_dir)?;
    if !config.enabled {
    ...
    let listener = tokio::net::TcpListener::bind(addr).await.map_err(|e| {
        if let Err(clear_err) = clear_status(&options.app_data_dir) {
            log::warn!("[mcp] failed to clear stale HTTP status after bind failure: {clear_err}");
        }
        e.to_string()
    })?;
    write_status(&options.app_data_dir, &config)?;
