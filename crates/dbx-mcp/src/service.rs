...
use crate::tools::{
    ...
    DescribeTableArgs, ExecuteAndShowArgs, ExecuteQueryArgs, ListDatabasesArgs, ListSchemasArgs, ListTablesArgs,
    OpenTableArgs, QUERY_ROW_LIMIT,
};
...
impl DbxMcpService {
    ...
    async fn dbx_execute_and_show(
        &self,
        Parameters(args): Parameters<ExecuteAndShowArgs>,
    ) -> Result<CallToolResult, McpError> {
        let config = self.find_connection(&args.connection_name).await?;
        let database = resolve_database(&config, args.database);
        self.events
            .execute_query(McpExecuteQueryEvent {
                connection_id: config.id,
                database,
                sql: args.sql,
                allow_writes: args.allow_writes.unwrap_or(false),
